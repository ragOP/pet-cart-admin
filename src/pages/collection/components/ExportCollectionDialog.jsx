...
import { exportCollections } from "../helpers/exportCollection";
...
const ExportCollectionDialog = ({
  ...
  const { mutate: exportMutation, isPending } = useMutation({
    mutationFn: async (type) => {
      ...
      const apiResponse = await exportCollections({ params: updatedParams });
      if (apiResponse?.response?.success) {
        ...
        link.setAttribute(
          "download",
          data.filename || (type === "csv" ? "collection.csv" : "collection.xlsx")
        );
