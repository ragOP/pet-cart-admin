...
import { Search, Image, Package, Folder, Layers, Tag, ArrowUp, AlertCircle } from "lucide-react";
...
const ContentSelector = ({ contentType, onSelect, currentItem }) => {
    ...
    const totalCount = response?.data?.total || response?.data?.length || contentData.length;
    ...
    const getDisplayName = (item, type) => {
        return contentTypeConfig[type]?.getDisplayName(item) || item.title || item.name || 'Unknown Item';
    };
    ...
    const handleSelect = (item) => {
        const selectedItem = {
            itemId: item._id,
            link: `/${contentType}/${item.slug || item._id}`,
            ...
            return (
                <div className="flex flex-col items-center justify-center min-h-[40vh] max-h-[80vh] py-12">
                    ...
                    <p className="text-gray-500 text-center max-w-md mb-6">
                        No {contentTypeConfig[contentType]?.plural || contentType} match your search for <span className="font-medium text-gray-700">"{searchTerm}"</span>
                    </p>
        ...
        if (hasNoData) {
            return (
                <div className="flex flex-col items-center justify-center min-h-[40vh] max-h-[80vh] py-12">
                    ...
                    <h3 className="text-xl font-semibold text-gray-900 mb-3">No {contentTypeConfig[contentType]?.plural || contentType} available</h3>
                    <p className="text-gray-500 text-center max-w-md mb-6">
                        There are currently no {contentTypeConfig[contentType]?.plural || contentType} in the database.
                        {contentType === 'product' && ' Products will appear here once they are added to your catalog.'}
                        {contentType === 'category' && ' Categories will appear here once they are created.'}
                        {contentType === 'subCategory' && ' Sub-categories will appear here once they are created.'}
                        {contentType === 'collection' && ' Collections will appear here once they are created.'}
                    </p>
                </div>
            );
        }
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <DialogTrigger asChild>
                ...
                >
                    {getIcon(contentType)}
                    <span className="ml-2">
                        {currentItem ? `Change ${contentTypeConfig[contentType]?.singular || contentType}` : `Select ${contentTypeConfig[contentType]?.singular || contentType}`}
                    </span>
                </Button>
            </DialogTrigger>
            <DialogContent className="max-w-7xl max-h-[90vh] overflow-hidden flex flex-col">
                <DialogHeader>
                    <DialogTitle>Select {contentTypeConfig[contentType]?.singular || contentType}</DialogTitle>
                </DialogHeader>
                    ...
                    {contentData.length > 0 && (
                        <div className="text-sm text-gray-500">
                            <div className="flex items-center gap-2">
                                <span>📊</span>
                                <span>Showing <strong>{showingCount}</strong> of <strong>{totalDisplayCount}</strong> {contentTypeConfig[contentType]?.plural || contentType}</span>
                            </div>
                        </div>
                    )}
                </div>
                ...
                <div className="flex-1 overflow-hidden">
                    ...
                    {loading ? (
                        <div className="flex flex-col items-center justify-center h-full py-12">
                            ...
                            <p className="text-gray-500 text-sm">Loading {contentTypeConfig[contentType]?.plural || contentType}s...</p>
                        </div>
                    ) : (
                        <>
                            {/* Content List */}
                                ...
                                >
                                            ...
                                            >
                                                <div className="flex items-center space-x-4">
                                                    ...
                                                    <div className="flex-shrink-0 w-16 h-16 bg-gray-100 rounded-md overflow-hidden">
                                                        ...
                                                        <div className="w-full h-full flex items-center justify-center" style={{ display: getImageUrl(item, contentType) ? 'none' : 'flex' }}>
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        {(() => {
                                                            const details = contentTypeConfig[contentType]?.getDetails(item);
                                                            return (
                                                                <>
                                                                    {details.price && (
                                    ...
                                    {showScrollTop && (
                                        <Button
                                            ...
                                            onClick={() => {
                                                const scrollContainer = document.querySelector('.min-h-\\[40vh\\]');
                                                if (scrollContainer) {
